    ...
    const std::string & type)
{
  ...
  if (type == "nv12_to_rgb8") {
    ...
    success = accelerator_.nv12_to_rgb8_opencv(input_fd, output_fd, alignd_width, alignd_height);
#else
...
#ifdef USE_OPENCV_BACKEND
    success = accelerator_.rgb8_to_nv12_opencv(input_fd, output_fd, alignd_width, alignd_height);
#else
